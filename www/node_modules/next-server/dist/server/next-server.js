...
class Server {
    constructor({ dir = '.', staticMarkup = false, quiet = false, conf = null } = {}) {
        ...
        if (process.env.NODE_ENV === 'production' && target !== 'server')
            throw new Error('Cannot start server when target is not server. https://err.sh/zeit/next.js/next-start-serverless');
    ...
    async sendHTML(req, res, html) {
        ...
        return send_html_1.sendHTML(req, res, html, { generateEtags });
    }
    async render(req, res, pathname, query = {}, parsedUrl) {
        ...
        if (this.nextConfig.poweredByHeader) {
            res.setHeader('X-Powered-By', 'Next.js ' + "8.0.0-canary.18");
        }
        return this.sendHTML(req, res, html);
    }
    async renderToHTMLWithComponents(req, res, pathname, query = {}, opts) {
        const result = await load_components_1.loadComponents(this.distDir, this.buildId, pathname);
    ...
    async renderErrorToHTML(err, req, res, _pathname, query = {}) {
        return this.renderToHTMLWithComponents(req, res, '/_error', query, Object.assign({}, this.renderOpts, { err }));
    }
